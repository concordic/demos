...
int _get_queue_family_index(VkPhysicalDevice physdev, bool (*check_feature)(VkPhysicalDevice, VkQueueFamilyProperties*, int, int)) {
	...
	for (int i = 0; i < num_queue_families; i++) {
		if (check_feature(physdev, queues, num_queue_families, i)) {
			return i;
		}
	}
	return -1;
...
VkDeviceQueueCreateInfo* queueCreatesInit(VkPhysicalDevice physdev, VkInstance instance, 
		bool(*check_features[])(VkPhysicalDevice, VkQueueFamilyProperties*, int, int), int num_features, int* num_queues) {
	...
	for (int i = 0; i < num_features; i++) {
		...
		*priority = 1.0f;
		int family_index = _get_queue_family_index(physdev, check_features[i]);
		for (int j = 0; j < i; j++) {
